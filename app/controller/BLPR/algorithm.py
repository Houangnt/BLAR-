...
from app.common.model import BLARResults, DetectionResult2
...
class Algorithm(AlgorithmBase):
    ...
    def process(self, org_img):
        # plate_results = self._plate_detector.detect([org_img])
        # if len(plate_results.results) == 0:
        #    return BLARResults(org_img=org_img, plate=[])
        # crop and draw bbox image with yolov5
        # sticker_images = preprocess_crop_rec(plate_results.results)
        # sticker_images = preprocess_draw_rec(plate_results.results)
        ...
        return DetectionResult2(org_img=org_img, plate=line_text_detection)
        # return BLARResults(org_img=org_img, plate=postprocessed_results)
